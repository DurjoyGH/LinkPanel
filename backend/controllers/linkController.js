...
const createLink = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ success: true, message: "Link created successfully.", link });
  } catch (error) {
...
const getLinkById = async (req, res) => {
  try {
    ...
    if (
      req.user.role !== "admin" &&
      link.createdBy._id.toString() !== req.user._id.toString()
    ) {
      return res
        .status(403)
...
const updateLink = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Link updated successfully.", link });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
